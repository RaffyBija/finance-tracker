...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    content: (
      <>
        <p>Crea budget per categoria con un importo massimo e un periodo (settimanale, mensile, annuale o <strong>periodo di paga</strong>, da uno stipendio al successivo). La barra di avanzamento mostra quanto hai già speso nel <strong>periodo corrente</strong> rispetto al limite.</p>
        ...
        <p>In cima alla pagina una <strong>fascia di riepilogo</strong> mostra a colpo d'occhio quanto hai <strong>budgetato, speso e rimanente</strong> tra i budget attivi, più lo <strong>spendibile del periodo</strong>.</p>
        <p>Con <strong>«Proponi budget»</strong> l'app calcola lo <strong>spendibile del periodo</strong> (di paga o mese): quanto avevi a inizio periodo, al netto del debito delle carte, più le entrate, meno le spese <strong>fisse e programmate</strong> (ricorrenti, pianificate, rate, anche su carta) e la tua <strong>% di risparmio</strong> sulle entrate. Le spese contano alla data d'acquisto, come nell'Analisi; per il periodo in corso vedi anche quanto hai già speso in variabili. Poi suggerisce un tetto per categoria dalla tua <strong>spesa variabile media</strong> (niente tetti su fisse e rate, già considerate). Scegli se pianificare il periodo <strong>in corso o il successivo</strong> (vicino alla fine ti propone già il successivo, stimando le spese fino ad allora) e quali <strong>conti includere</strong>. I budget applicati seguono il periodo scelto. La percentuale di risparmio si imposta in <strong>Impostazioni → Preferenze</strong>.</p>
      </>
    ),
  },
  {
