...
import { useDueReview } from '../due/DueReviewProvider';
import { rataToEntry, todayLocal } from '../due/dueEntries';
...
export default function InstallmentPlansSection() {
  ...
  // "Paga" dal dettaglio apre il popup unico "Scadenze da registrare" sulle rate
  // scelte: data prevista correggibile, conto richiesto se il piano è a pagamento
  // manuale; rate con stessa data → una transazione.
  const handlePayFromDetail = (plan: InstallmentPlan, ids: string[]) => {
    ...
    openDueEntries(
      plan.installments
        ...
        .map((r) => rataToEntry(r, plan, today)),
    );
  };
